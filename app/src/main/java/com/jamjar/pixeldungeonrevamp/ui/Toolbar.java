 ...
 */
package com.jamjar.pixeldungeonrevamp.ui;
...
import com.jamjar.pixeldungeonrevamp.windows.WndCatalogus;
...
public class Toolbar extends Component {
	...
	protected void layout() {
		...
		int slots = PixelDungeonRevamp.quickSlots();
		...
		for(int i = 0; i <= 3; i++) {
			...
			if (slots == 4 && width < 150){
				if (width < 139){
					if ((PixelDungeonRevamp.flipToolbar() && i == 3) ||
							(!PixelDungeonRevamp.flipToolbar() && i == 0)) {
					...
					if (i == 0 && !PixelDungeonRevamp.flipToolbar() ||
						i == 3 && PixelDungeonRevamp.flipToolbar()){
						btnQuick[i].border(0, 2);
						btnQuick[i].frame(106, 0, 19, 24);
					} else if (i == 0 && PixelDungeonRevamp.flipToolbar() ||
							i == 3 && !PixelDungeonRevamp.flipToolbar()){
		...
		float right = width;
		switch(Mode.valueOf(PixelDungeonRevamp.toolbarMode())){
			case SPLIT:
				btnWait.setPos(x, y);
		...
		if (PixelDungeonRevamp.flipToolbar()) {
