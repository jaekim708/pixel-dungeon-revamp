...
import com.jamjar.pixeldungeonrevamp.effects.BannerSprites.Type;
import com.jamjar.pixeldungeonrevamp.effects.Speck;
import com.jamjar.pixeldungeonrevamp.messages.Messages;
import com.jamjar.pixeldungeonrevamp.ui.Archs;
import com.jamjar.pixeldungeonrevamp.ui.ExitButton;
import com.jamjar.pixeldungeonrevamp.ui.Icons;
import com.jamjar.pixeldungeonrevamp.ui.RedButton;
import com.jamjar.pixeldungeonrevamp.ui.RenderedTextMultiline;
import com.jamjar.pixeldungeonrevamp.windows.WndChallenges;
import com.jamjar.pixeldungeonrevamp.windows.WndClass;
...
public class StartScene extends PixelScene {
	...
	public void create() {
		...
		float width, height;
		if (PixelDungeonRevamp.landscape()) {
			width = WIDTH_L;
			height = HEIGHT_L;
		} else {
			width = WIDTH_P;
		...
		HeroClass[] classes = {
				HeroClass.WARRIOR, HeroClass.MAGE, HeroClass.ROGUE, HeroClass.HUNTRESS, HeroClass.PIRATE
		};
		...
		if (PixelDungeonRevamp.landscape()) {
			...
			float shieldW = width / NUM_COLS;
			float shieldH = Math.min( centralHeight / NUM_ROWS, shieldW * 1.8f );
			top = title.y + title.height() + centralHeight / NUM_ROWS - shieldH;
			for (int i=0; i < classes.length; i++) {
				ClassShield shield = shields.get( classes[i] );
				shield.setRect(
						left + (i % NUM_COLS) * shieldW,
						top + (i / NUM_COLS) * shieldH,
						shieldW, shieldH );
				align(shield);
			}
		...
		curClass = null;
		updateClass( HeroClass.values()[PixelDungeonRevamp.lastClass()] );
		...
		Badges.loadingListener = new Callback() {
			...
			public void call() {
				if (Game.scene() == StartScene.this) {
					PixelDungeonRevamp.switchNoFade( StartScene.class );
				}
			}
		};
	}
	...
	private void startNewGame() {
		...
		if (PixelDungeonRevamp.intro()) {
			PixelDungeonRevamp.intro( false );
	...
	protected void onBackPressed() {
		PixelDungeonRevamp.switchNoFade( TitleScene.class );
	}
	...
	private class ClassShield extends Button {
		...
		public ClassShield( HeroClass cl ) {
			...
			if (cl.ordinal() == 4)
				avatar.frame( 2 * WIDTH, 0, WIDTH, HEIGHT );
			else
	...
	private class ChallengeButton extends Button {
		...
		protected void createChildren() {
			...
			image = Icons.get( PixelDungeonRevamp.challenges() > 0 ? Icons.CHALLENGE_ON :Icons.CHALLENGE_OFF );
			add( image );
		}
		...
		protected void onClick() {
			if (Badges.isUnlocked( Badges.Badge.VICTORY )) {
				StartScene.this.add(new WndChallenges(PixelDungeonRevamp.challenges(), true) {
					public void onBackPressed() {
						super.onBackPressed();
						image.copy( Icons.get( PixelDungeonRevamp.challenges() > 0 ?
