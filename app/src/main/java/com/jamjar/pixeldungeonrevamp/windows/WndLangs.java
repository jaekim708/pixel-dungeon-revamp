...
import com.jamjar.pixeldungeonrevamp.scenes.TitleScene;
...
public class WndLangs extends Window {
	...
	public WndLangs(){
		...
		for (int i = 0; i < langs.size(); i++){
			...
			RedButton btn = new RedButton(Messages.titleCase(langs.get(i).nativeName())){
				...
				protected void onClick() {
					...
					PixelDungeonRevamp.switchNoFade(TitleScene.class, new Game.SceneChangeCallback() {
						@Override
						public void beforeCreate() {
							PixelDungeonRevamp.language(langs.get(langIndex));
			...
			if (PixelDungeonRevamp.landscape() && i % 2 == 1){
		...
		resize(PixelDungeonRevamp.landscape() ? WIDTH_L : WIDTH_P, y);
